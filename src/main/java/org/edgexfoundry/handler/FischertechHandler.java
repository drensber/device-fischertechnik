...
@Service
public class FischertechHandler {

	private final static EdgeXLogger logger = EdgeXLoggerFactory.getEdgeXLogger(FischertechHandler.class);
	
	@Autowired
	private FischertechDriver driver;
	...
	@Value("${fischertech.device.init:#{null}}")
	private String fischertechInit;
	@Value("${fischertech.device.init.args:#{null}}")
	private String fischertechInitArgs;
	
	@Value("${fischertech.device.remove:#{null}}")
	private String fischertechRemove;
	@Value("${fischertech.device.remove.args:#{null}}")
	private String fischertechRemoveArgs;
	...
	public void initializeDevice(Device device) {
		...
		FischertechDevice.setFailures(0);
		if(fischertechInit != null && commandExists(device, fischertechInit))
			executeCommand(FischertechDevice, fischertechInit, fischertechInitArgs);
	...
	private void executeOperations(FischertechDevice device, String commandName, String arguments, Boolean immediate, String transactionId) {
		String method = (arguments == null) ? "get" : "set";
		
		String deviceName = device.getName();
		String deviceId = device.getId();
		// get the objects for this device
		Map<String, FischertechObject> objects = profiles.getObjects().get(deviceName);
	...
	private Boolean requiresQuery(boolean immediate, String method, FischertechDevice device, ResourceOperation operation) {
	...
	private FischertechObject getFischertechObject(Map<String, FischertechObject> objects, String objectName, String transactionId) {
		FischertechObject object = objects.get(objectName);
